...
def build_paths_tab(window: ConfigWindow, i18n: dict) -> QWidget:
    ...
    layout.addRow(
        ...
        window._create_browse_widget(
            window.folder_to_watch_edit, QFileDialog.FileMode.Directory
        ),
    )
    ...
    output_folder_layout.addWidget(
        window._create_browse_button(
            window.output_folder_edit, QFileDialog.FileMode.Directory
        )
    )
    output_folder_layout.addWidget(window.copy_temp_files_to_output_folder_check)
    output_folder_layout.addWidget(
        window._create_labeled_widget(
            tabs_i18n, "paths", "copy_temp_files_to_output_folder"
        )
    )
    ...
    layout.addRow(
        window._create_labeled_widget(
            tabs_i18n, "paths", "copy_trimmed_replay_to_output_folder"
        ),
        window.copy_trimmed_replay_to_output_folder_check,
    )
    layout.addRow(
        window._create_labeled_widget(
            tabs_i18n, "paths", "open_output_folder_on_card_creation"
        ),
