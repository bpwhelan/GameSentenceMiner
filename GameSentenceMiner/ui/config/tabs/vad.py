...
def build_vad_tab(window: ConfigWindow, i18n: dict) -> QWidget:
    ...
    layout.addRow(
        window._create_labeled_widget(tabs_i18n, "vad", "do_postprocessing"),
        window.do_vad_postprocessing_check,
    )

    models_group = window._create_group_box("VAD Models")
    models_layout = QFormLayout()
    models_layout.addRow(
        window._create_labeled_widget(tabs_i18n, "vad", "whisper_model"),
        window.whisper_model_combo,
    )
    models_layout.addRow(
        window._create_labeled_widget(tabs_i18n, "vad", "selected_model"),
        window.selected_vad_model_combo,
    )
    models_layout.addRow(
        window._create_labeled_widget(tabs_i18n, "vad", "backup_model"),
        window.backup_vad_model_combo,
    )
    models_layout.addRow(
        window._create_labeled_widget(tabs_i18n, "vad", "use_cpu_for_inference"),
        window.use_cpu_for_inference_check,
    )
    models_layout.addRow(
    ...
    layout.addRow(
        window._create_labeled_widget(tabs_i18n, "vad", "add_on_no_results"),
        window.add_audio_on_no_results_check,
    )
    layout.addRow(
