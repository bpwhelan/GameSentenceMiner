...
class ConfigWindow(QWidget):
    ...
    def save_settings(
        ...
        try:
            ...
            sources_editor = getattr(self, "general_websocket_sources_editor", None) or getattr(
                self, "req_websocket_sources_editor", None
            )
            if sources_editor:
            ...
            config = ProfileConfig(
                ...
                anki=Anki(
                    ...
                    tags_to_check=[
                        tag.strip().lower() for tag in self.tags_to_check_edit.text().split(",") if tag.strip()
                    ],
                ...
                obs=OBS(
                    ...
                    replay_buffer_enabled=self.obs_replay_buffer_enabled_check.isChecked(),
                    replay_buffer_duration_seconds=self.obs_replay_buffer_duration_spin.value(),
            ...
            self.master_config.experimental = Experimental(
                ...
                enable_hoshidicts=self.enable_hoshidicts_check.isChecked(),
    ...
    def _create_all_widgets(self):
        ...
        self.obs_replay_buffer_enabled_check = QCheckBox()
        self.obs_replay_buffer_duration_spin = QSpinBox()
        self.obs_replay_buffer_duration_spin.setRange(1, 86400)
        self.obs_replay_buffer_duration_spin.setValue(300)
        ...
        self.enable_hoshidicts_check = QCheckBox()
    ...
    def _populate_gsm_cloud_models(self, selected_models: list[str] | None) -> None:
        ...
        try:
            ...
            for model_name in supported_models:
                ...
                item.setCheckState(Qt.CheckState.Checked if model_name in normalized else Qt.CheckState.Unchecked)
    ...
    def _on_gsm_cloud_sync_now_clicked(self) -> None:
        ...
        include_existing = bool(
            int(sync_status.get("since_seq", 0) or 0) == 0 and int(sync_status.get("pending_changes", 0) or 0) == 0
        )
    ...
    def _on_gsm_cloud_sync_finished(self, result: dict) -> None:
        ...
        if status == "success":
            ...
            rollup_status = (
                "queued" if rollup_triggered is True else "failed" if rollup_triggered is False else "not requested"
            )
        ...
        reason = str(result.get("last_error") or result.get("reason") or "Unknown error.").strip()
        if status == "skipped":
    ...
    def _connect_signals(self):
        ...
        self.auto_accept_timer_enabled_check.stateChanged.connect(self._sync_auto_accept_timer_controls)
    ...
    def _sync_auto_accept_timer_controls(self):
        ...
        if enabled:
            ...
            if not timer_text or timer_text == "0":
                self.auto_accept_timer_edit.setText(str(DEFAULT_AUTO_ACCEPT_TIMER_SECONDS))
    ...
    def _refresh_anki_model_list(self, preserve_selection=True):
        ...
        if hasattr(self, "req_note_type_combo"):
            ...
            selected_req = (
                current_req if preserve_selection and current_req in models else (models[0] if models else "")
            )
            if selected_req:
    ...
    def load_settings_to_ui(self):
        ...
        self.auto_accept_timer_edit.setText(
            str(s.anki.auto_accept_timer if auto_accept_timer_enabled else DEFAULT_AUTO_ACCEPT_TIMER_SECONDS)
        )
        ...
        self.obs_replay_buffer_enabled_check.setChecked(getattr(s.obs, "replay_buffer_enabled", True))
        self.obs_replay_buffer_duration_spin.setValue(
            max(1, min(86400, getattr(s.obs, "replay_buffer_duration_seconds", 300)))
        )
        ...
        self.enable_hoshidicts_check.setChecked(getattr(experimental_cfg, "enable_hoshidicts", False))
    ...
    def delete_profile(self):
        ...
        if profile_to_delete == DEFAULT_CONFIG:
            QMessageBox.critical(
                ...
                i18n.get("error_cannot_delete_default", "Cannot delete Default profile."),
            )
            return
    ...
    def refresh_ai_models(self, provider=None):
        ...
        if provider == "gemini":
            ...
            self.groq_backup_model_combo.addItems(
                [OFF] + [m for m in groq_models if m not in {OFF, "RECOMMENDED", "OTHER"}]
            )
