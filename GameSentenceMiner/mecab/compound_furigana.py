    ...
    for word_idx in range(start_index, len(expr.word)):
        for reading_idx in range(start_index, len(expr.reading)):
            if expr.word[word_idx] == expr.reading[reading_idx]:
                prefix_len = find_common_prefix_len(
                    expr.word[word_idx:], expr.reading[reading_idx:]
                )
...
def break_compound_furigana_chunk(expr: str) -> str:
    if (d := dismember(expr)) and (c := find_common_kana(d)):
        return (
            f"{c.first.assemble()} {break_compound_furigana_chunk(c.second.assemble())}"
        )
    else:
        return expr
...
if __name__ == "__main__":
    assert dismember("相合い傘[あいあいがさ]") == Dismembered(
        word="相合い傘", reading="あいあいがさ", tail=""
    )
    ...
    assert (
        break_compound_furigana("相合い傘[あいあいがさ]") == "相合[あいあ]い 傘[がさ]"
    )
    print("Done.")
