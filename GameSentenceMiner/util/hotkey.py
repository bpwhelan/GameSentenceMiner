...
class HotkeyManager:
    def __init__(self):
        ...
        if current_os == "Windows":
            ...
            self.mode = "disabled"
            logging.warning("HotkeyManager: Non-Windows OS detected but 'pynput' not installed.")
    ...
    def register(self, hotkey_str, callback):
        if not hotkey_str or self.mode == "disabled":
            return
        ...
        def debounced_wrapper():
            ...
            if self._lock.acquire(blocking=False):
                try:
                    ...
                    logging.error(f"Error in hotkey callback for {hotkey_str}: {e}")
                finally:
                    self._lock.release()
        ...
        if self.mode == "keyboard":
            try:
                ...
                logging.error(f"Failed to register Windows hotkey '{hotkey_str}': {e}")
            ...
            try:
                ...
                logging.error(f"Failed to register pynput hotkey '{translated_key}': {e}")
