...
supported_formats = {
    ...
    "aac": {
        "codec": "aac",
        "format": "adts",
    },  # ADTS is the proper container for raw AAC
    "m4a": {
        "codec": "aac",
        "format": "ipod",
    },  # iPod/M4A format for AAC in MP4 container
}
...
class FFmpegHelper:
    ...
    def get_probe_json(file_path: str, entries: str, stream_select: str) -> Optional[dict]:
        ...
        cmd.extend(["-show_entries", entries, "-of", "json", str(file_path)])
...
def call_frame_extractor(video_path, timestamp):
    ...
    try:
        ...
        return dialog_manager.screenshot_selector_sync(
            video_path,
            str(timestamp),
            get_config().screenshot.screenshot_timing_setting,
        )
    except Exception as e:
...
def encode_screenshot(input_image, source_video_path=None, screenshot_timing=None, output_path=None):
    ...
    if output_path is None:
        output_image = make_unique_file_name(
            os.path.join(
                get_temporary_directory(),
                f"{obs.get_current_game(sanitize=True)}.{get_config().screenshot.extension}",
            )
        )
    else:
...
def get_screenshot_time(
    video_path,
    game_line,
    default_beginning=False,
    vad_result=None,
    doing_multi_line=False,
    previous_line=False,
    anki_card_creation_time=0,
):
    if game_line:
    ...
    if get_config().screenshot.screenshot_timing_setting == "beginning":
        ...
        if game_line and game_line.next_line():
            screenshot_time_from_beginning = (
                line_timestamp_in_video
                + ((game_line.next_line().time - game_line.time).total_seconds() / 2)
                + screenshot_offset
            )
        else:
            screenshot_time_from_beginning = (
                file_length - ((file_length - line_timestamp_in_video) / 2)
            ) + screenshot_offset
    elif get_config().screenshot.screenshot_timing_setting == "end":
        if game_line and game_line.next_line():
            screenshot_time_from_beginning = (
                line_timestamp_in_video
                + (game_line.next_line().time - game_line.time).total_seconds()
                - screenshot_offset
            )
        else:
...
def trim_audio_based_on_last_line(untrimmed_audio, video_path, game_line, next_line, anki_card_creation_time):
    ...
    if source_padding:
        logger.debug(
            f"Applied source padding of {source_padding:.2f}s for audio start trim (source: {getattr(game_line, 'source', None)})"
        )
    logger.success(
        f"Audio Extracted and trimmed to {start_trim_time} seconds"
...
def reencode_file_with_user_config(input_file, final_output_audio, user_ffmpeg_options):
    ...
    if not final_output_audio:
        final_output_audio = make_unique_file_name(
            os.path.join(
                get_temporary_directory(),
...
def trim_audio(
    ...
    if fade_filter:
        command.extend(
            ["-af", ",".join(fade_filter)]
        )  # Fixed: Join multiple filters if both exist? Original had duplicate -af calls or combined string logic that was slightly off or just overwrote.
    ...
    try:
        for i, (start, end) in enumerate(keep_ranges):
            seg_name = make_unique_file_name(
                os.path.join(
                    get_temporary_directory(),
                    f"segment_{i}_{os.path.basename(input_audio)}",
                )
            )
