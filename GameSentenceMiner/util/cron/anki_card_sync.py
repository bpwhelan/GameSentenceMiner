...
def _fetch_and_upsert_notes(note_ids: list[int], *, strict: bool = False) -> int:
    """Batch-fetch ``notesInfo`` from AnkiConnect and upsert into ``anki_notes``.

    Args:
        note_ids: List of Anki note IDs to fetch.
        strict: When True, abort on batch failures or missing requested notes.
    ...
    for i in range(0, len(note_ids), _NOTES_BATCH_SIZE):
        ...
        if strict:
            missing_note_ids = {int(note_id) for note_id in batch} - returned_note_ids
            if missing_note_ids:
                raise RuntimeError(
                    "notesInfo did not return requested notes: "
                    + ", ".join(str(note_id) for note_id in sorted(missing_note_ids))
                )
...
def _fetch_and_upsert_cards(card_ids: list[int], *, strict: bool = False) -> int:
    """Batch-fetch ``cardsInfo`` from AnkiConnect and upsert into ``anki_cards``.

    Args:
        card_ids: List of Anki card IDs to fetch.
        strict: When True, abort on batch failures or missing requested cards.
    ...
    for i in range(0, len(card_ids), _CARDS_BATCH_SIZE):
        ...
        if strict:
            missing_card_ids = {int(card_id) for card_id in batch} - returned_card_ids
            if missing_card_ids:
                raise RuntimeError(
                    "cardsInfo did not return requested cards: "
                    + ", ".join(str(card_id) for card_id in sorted(missing_card_ids))
                )
...
def _fetch_and_upsert_reviews(card_ids: list[int], *, strict: bool = False) -> int:
    ...
    Args:
        ...
        strict: When True, abort on batch failures or unmapped cards.
...
def _delete_stale_rows(
    ...
    optionally cached card IDs against *live_card_ids*. Removes stale entries
    from all five cache tables in the correct order to respect
    ...
    Deletion order (children first):
      ...
      4. anki_cards        (by card_id)
      5. anki_notes         (by note_id)

    Args:
        live_note_ids: The complete set of note IDs currently in Anki.
        live_card_ids: The complete set of card IDs currently in Anki. When
            provided, cards removed from still-live notes are also purged.

    Returns:
        Dict with keys ``stale_notes``, ``stale_cards``,
        ``deleted_card_kanji_links``,
    ...
    if stale_note_ids or live_card_ids is not None:
        ...
        for raw_card_id, raw_note_id in rows:
            try:
    ...
    result["stale_cards"] = len(stale_card_ids)
    stale_card_list = list(stale_card_ids)
    ...
    logger.info(
        ...
        f"{result['stale_cards']} cards, "
        f"{result['deleted_cards']} deleted cards, "
        f"{result['deleted_reviews']} reviews, "
