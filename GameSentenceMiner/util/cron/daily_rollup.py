...
def analyze_hourly_data(lines: List) -> Dict:
    ...
    """
    if not lines:
        return {"hourly_activity": {}, "hourly_speeds": {}}
...
def analyze_game_activity(lines: List, date_str: str) -> Dict:
    ...
    """
    if not lines:
        return {"completed": 0, "started": 0, "details": {}, "game_ids": []}
    ...
    for game_id, data in game_data.items():
        ...
        try:
            ...
            if data["game_name"]:
                title = data["game_name"]
                logger.info(
                    f"[ROLLUP_TITLE] Exception during lookup, using game_name '{title}' for {game_id[:8]}...: {e}"
                )
            else:
                title = f"Game {game_id[:8]}"
                logger.warning(
                    f"[ROLLUP_TITLE] Exception and no game_name for {game_id[:8]}..., using shortened UUID: {e}"
                )
...
def run_daily_rollup() -> Dict:
    ...
    try:
        ...
        logger.info(
            f"Date range: {first_date} to {end_date}, Total dates: {total_dates}, Processed: {processed}, Overwritten: {overwritten}, Errors: {errors}, Time: {elapsed_time:.2f}s"
        )
