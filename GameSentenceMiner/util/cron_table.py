...
class CronTable(SQLiteDBTable):
    ...
    _table = 'cron_table'
    _fields = [
        'name', 'description', 'last_run', 'next_run', 
        'enabled', 'created_at', 'schedule'
    ]
    _types = [
        int,      # id (primary key)
        str,      # name
    ...
    def __init__(
        ...
        self.schedule = schedule if schedule in ['once', 'daily', 'weekly', 'monthly', 'yearly'] else 'once'
    ...
    def get_by_name(cls, name: str) -> Optional['CronTable']:
        ...
        row = cls._db.fetchone(
            f"SELECT * FROM {cls._table} WHERE name=?", (name,)
        )
