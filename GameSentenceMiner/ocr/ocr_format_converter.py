...
def convert_ocr_result_to_unified_format(
    ocr_result: Any, engine_name: str = "unknown"
        ...
        if isinstance(ocr_result, tuple) and len(ocr_result) == 6:
            ...
            if isinstance(element2, list) and len(element2) > 0:
                if isinstance(element2[0], dict) and "bounding_rect" in element2[0]:
                    logger.debug(
                        f"convert_ocr_result_to_unified_format: {engine_name} - Using filtered_lines directly (element2)"
                    )
                    return element2
            ...
            if isinstance(response_dict, dict) and "lines" in response_dict:
                ...
                if lines and isinstance(lines[0], dict) and "bounding_rect" in lines[0]:
                    logger.debug(
                        f"convert_ocr_result_to_unified_format: {engine_name} - Using lines from response_dict"
                    )
                    return lines
...
def extract_from_api_response(
    api_response: Dict[str, Any], engine_name: str = "unknown"
) -> Optional[List[Dict[str, Any]]]:
    """
...
def extract_from_google_lens_protobuf_response(
...
def extract_from_google_lens_json_response(
    response: Dict[str, Any],
) -> List[Dict[str, Any]]:
    """
...
def convert_normalized_coords_to_pixels(
    lines: List[Dict[str, Any]], img_width: int, img_height: int
) -> List[Dict[str, Any]]:
    """
