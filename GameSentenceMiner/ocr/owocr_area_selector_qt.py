...
class OWOCRAreaSelectorWidget(QWidget):
    ...
    def _initialize(self):
        ...
        try:
            ...
            qimage = QImage(
                img_data,
                img_to_convert.width,
                img_to_convert.height,
                img_to_convert.width * 3,
    ...
    def _init_monitor_capture(self):
        ...
        with mss.mss() as sct:
            ...
            if mss_idx >= len(sct.monitors):
                logger.error(
                    f"Monitor index {target_idx} out of range (Found {len(sct.monitors) - 1} monitors). Using primary."
                )
    ...
    def _init_obs_screenshot(self):
        ...
        if not self.screenshot_img:
            raise RuntimeError(
                "Failed to get OBS screenshot after multiple retries. Is the game running and visible in OBS?"
            )
    ...
    def _init_window_capture(self):
        ...
        self._fit_capture_to_screen(original_w, original_h)
        logger.info(
            f"Captured {original_w}x{original_h} desktop area (scaled to {self.screenshot_img.width}x{self.screenshot_img.height})"
        )
        ...
        if event.button() == Qt.MouseButton.LeftButton and self.is_drawing:
            ...
            if w >= MIN_RECT_WIDTH and h >= MIN_RECT_HEIGHT:
                ...
                for i, mon in enumerate(self.monitors):
                    if (
                        mon["left"] <= rect_center_x < mon["left"] + mon["width"]
                        and mon["top"] <= rect_center_y < mon["top"] + mon["height"]
                    ):
                        monitor_index = i
                        break
    ...
    def save_and_quit(self):
        ...
        if self.select_monitor_area:
            ...
            try:
                ...
                logger.success(
                    f"Saved {len(final_rects)} monitor regions and index {self.target_monitor_index} to {out_path}"
                )
            except Exception as e:
        ...
        for rect in self.rectangles:
            ...
            monitor = next(
                (m for m in self.monitors if m["index"] == rect["monitor_index"]),
                self.monitors[0],
            )
