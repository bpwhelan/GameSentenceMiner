...
async function loadGamesForBulkOperations() {
    ...
    try {
        ...
        if (response.ok && data.games) {
            ...
            if (searchInput) {
                searchInput.value = '';
                searchInput.removeEventListener('input', filterAndDisplayBulkGames);
                searchInput.addEventListener('input', filterAndDisplayBulkGames);
            }

            if (sortSelect) {
                sortSelect.value = 'recent';
                sortSelect.removeEventListener('change', filterAndDisplayBulkGames);
                sortSelect.addEventListener('change', filterAndDisplayBulkGames);
            }

            // Display games with default sorting
            filterAndDisplayBulkGames();
            content.style.display = 'block';
        }
    } catch (error) {
...
/**
 * Filter and display games based on search and sort criteria
 */
function filterAndDisplayBulkGames() {
    const searchInput = document.getElementById('gameSearchInput');
    const sortSelect = document.getElementById('gameSortSelect');
    const gamesList = document.getElementById('bulkGamesList');

    if (!allBulkGamesData || !gamesList) return;

    const searchTerm = searchInput ? searchInput.value.toLowerCase() : '';
    const sortBy = sortSelect ? sortSelect.value : 'recent';

    // Filter games by search term
    let filteredGames = allBulkGamesData.filter(game =>
