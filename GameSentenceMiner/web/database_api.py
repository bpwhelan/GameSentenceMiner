...
def register_database_api_routes(app):
    ...
    @app.route('/api/debug-streak', methods=['GET'])
    def api_debug_streak():
        """
        Debug endpoint to analyze streak calculation issues.
        """
        try:
            # Get all lines
            ...
            # Calculate daily reading times for debugging
            daily_reading_time = calculate_daily_reading_time(all_lines)
            
            # Get recent days for analysis
            ...
            for i in range(7):  # Last 7 days
                ...
                qualifies = reading_hours >= streak_requirement_hours
                
                recent_days.append({
                    'date': date_str,
                    'reading_hours': round(reading_hours, 4),
                    'reading_minutes': round(reading_hours * 60, 2),
                    'reading_seconds': round(reading_hours * 3600, 1),
                    'qualifies_for_streak': qualifies,
                    'requirement_hours': streak_requirement_hours
                })
            ...
            current_game_streak = calculate_time_based_streak(current_game_lines)
            all_games_streak = calculate_time_based_streak(all_lines)
            
            return jsonify({
                'config_values': {
                    'streak_requirement_hours': streak_requirement_hours,
                    'streak_requirement_minutes': round(streak_requirement_hours * 60, 2),
                    'streak_requirement_seconds': round(streak_requirement_hours * 3600, 1),
                    'afk_timer_seconds': afk_timer_seconds
                },
                'calculated_streaks': {
                    'current_game_streak': current_game_streak,
                    'all_games_streak': all_games_streak
                },
                'recent_days_analysis': recent_days,
                'total_lines': len(all_lines),
                'current_game_lines': len(current_game_lines),
                'total_daily_reading_days': len(daily_reading_time),
                'days_with_reading': len([d for d in daily_reading_time.values() if d > 0])
            }), 200
            
        except Exception as e:
            logger.error(f"Error in debug streak: {e}")
            return jsonify({'error': f'Debug failed: {str(e)}'}), 500

    @app.route('/api/debug-characters', methods=['GET'])
    def api_debug_characters():
        """
        Debug endpoint to analyze character counting discrepancies.
        """
        try:
            # Get a sample of recent lines
            all_lines = GameLinesTable.all()
            if not all_lines:
                return jsonify({'error': 'No lines found'}), 404
            
            # Get last 10 lines for analysis
            recent_lines = sorted(all_lines, key=lambda line: float(line.timestamp))[-10:]
            ...
            for line in recent_lines:
                ...
                debug_data.append({
                    ...
                    'text_preview': line_text[:50] + "..." if len(line_text) > 50 else line_text,
                    'character_count': len(line_text),
                    'text_length_breakdown': {
                        'total_chars': len(line_text),
                        'visible_chars': len(line_text.strip()),
                        'japanese_chars': sum(1 for c in line_text if '\u3040' <= c <= '\u309F' or '\u30A0' <= c <= '\u30FF' or '\u4E00' <= c <= '\u9FAF'),
                        'whitespace_chars': len(line_text) - len(line_text.replace(' ', '').replace('\t', '').replace('\n', '')),
                        'punctuation_estimate': sum(1 for c in line_text if c in '。、！？.,!?'),
                    },
                    'raw_text_repr': repr(line_text)  # Shows hidden characters
                })
            ...
            # Overall statistics
            total_chars = sum(len(line.line_text) if line.line_text else 0 for line in all_lines)
            total_lines = len(all_lines)
            
            return jsonify({
                'total_lines': total_lines,
                'total_characters': total_chars,
                'average_chars_per_line': total_chars / total_lines if total_lines > 0 else 0,
                'recent_lines_sample': debug_data,
                'duplicate_texts_found': len(duplicates),
                'top_duplicates': sorted(duplicates.items(), key=lambda x: x[1], reverse=True)[:5],
                'potential_issues': {
                    'has_duplicates': len(duplicates) > 0,
                    'unusually_long_lines': len([line for line in all_lines if line.line_text and len(line.line_text) > 200]),
                    'empty_lines': len([line for line in all_lines if not line.line_text or not line.line_text.strip()])
                }
            }), 200
            
        except Exception as e:
            logger.error(f"Error in debug characters: {e}")
            return jsonify({'error': f'Debug failed: {str(e)}'}), 500
