...
def register_jiten_database_api_routes(app):
        ...
        try:
            ...
            for row in game_names_from_lines:
                ...
                if game_name not in existing_titles:
                    ...
                    logger.debug(f"Auto-linked game_lines for: {game_name} -> game_id={game.id}")
        ...
        try:
            ...
            if not obs_scene_name:
                ...
                if result and result[0]:
                    ...
                    logger.warning(f"Could not find game_name in game_lines for game_id={game_id}, falling back to title_original")
    ...
    def api_repull_game_from_jiten(game_id):
        ...
        try:
            ...
            logger.error(f"Error repulling jiten data for game {game_id}: {e}", exc_info=True)
    ...
    def api_delete_individual_game(game_id):
        ...
        try:
            ...
            logger.debug(f"Unlinked game '{game_name}' (id={game_id}): removed game record, unlinked {unlinked_lines} lines")
