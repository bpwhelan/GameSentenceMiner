...
def get_recent_games(desired_count: int = 3, max_search: int = 50) -> List[GamesTable]:
    ...
    if valid_games:
        logger.info(
            f"Yomitan: SUCCESS - Found {len(valid_games)} game(s) with valid character data out of {len(checked_games)} checked"
        )
    else:
        logger.warning(
            f"Yomitan: FAILED - No games with valid character data found (checked {len(checked_games)} games)"
        )
...
def register_yomitan_api_routes(app):
    ...
    def generate_yomitan_dict():
        ...
        for game in recent_games:
            ...
            logger.debug(
                f"Yomitan: Added {char_count} characters from {game.title_original or game.title_romaji or 'Unknown'}"
            )
        ...
        logger.info(
            f"Yomitan: Generated dictionary with {len(builder.entries)} total entries from {total_characters} characters across {len(recent_games)} games"
        )
