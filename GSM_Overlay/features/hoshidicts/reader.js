...
(function (root, factory) {
  ...
  /**
   * Projects term results to a single explicitly selected dictionary: each
   * result keeps only glossaries whose `dictionary` matches `title`, and
   * results left with no matching glossary are dropped. The source results and
   * their glossary arrays are never mutated, so a cached term view they came
   * from stays intact for a Back action. Mirrors
   * projectHoshidictsResultsToSelectedDictionary in the shared feature module.
   */
  function projectResultsToSelectedDictionary(results, title) {
    const projected = [];
    for (const result of results) {
      const glossaries = result.term.glossaries.filter(
        (glossary) => glossary.dictionary === title
      );
  ...
  function dictionaryPresentationEqual(left, right) {
    return left.length === right.length && left.every((entry, index) => {
      ...
      return entry.title === other.title &&
        ...
        entry.termCount === other.termCount &&
        entry.kanjiCount === other.kanjiCount;
    });
  }
  ...
  function popupImageSourceEqual(left, right) {
  ...
  function createHoshidictsReader(options = {}) {
    ...
    // Title of the explicitly selected generic term dictionary when a clicked
    // kanji is routed through the term path (kanjiCount === 0 && termCount > 0);
    // null for Automatic clicks and kanji-bank selections.
    let latestKanjiTermDictionary = null;
    ...
    /**
     * The ordered list of dictionary titles permitted to show images for the
     * current popup image source, or null when Automatic (every dictionary is
     * permitted). A dictionary source yields its single title; a tab-group
     * source yields the group's members in their established order; a missing
     * or deleted group yields an empty allowlist so no images are shown. This
     * mirrors resolveHoshidictsPopupImageSourceDictionaries in the shared spec,
     * re-implemented here because the overlay cannot import shared TS.
     */
    function popupImageSourceDictionaries() {
      const source = preferences.popupImageSource;
      if (!isRecord(source)) {
        return null;
      }
      if (source.kind === "dictionary" && typeof source.title === "string") {
        return [source.title];
      }
      if (source.kind === "tabGroup" && typeof source.id === "string") {
        const groups = Array.isArray(preferences.dictionaryTabGroups)
          ? preferences.dictionaryTabGroups
          : [];
        const group = groups.find(
          (candidate) => isRecord(candidate) && candidate.id === source.id
        );
        return group && Array.isArray(group.dictionaries)
          ? [...group.dictionaries]
          : [];
      }
      return null;
    ...
    /**
     * Wraps the media resolver so only the chosen popup image source can show
     * images. Automatic delegates unchanged. An individual dictionary permits
     * only its own requests. A tab group shares one winner promise: image
     * references are probed in the group's established order (and within a
     * dictionary, in reference order) until the existing resolver succeeds; the
     * first dictionary with a resolvable image wins and only its requests are
     * allowed. Because the winning probe is cached by the resolver, the real
     * render is a cache hit, so bytes are never fetched twice; if no member
     * resolves, no image is shown.
     */
    function createGatedMediaResolver(results, targetDepth, generation) {
      const base = (request) => resolveMedia({ ...request, depth: targetDepth });
      const allowlist = popupImageSourceDictionaries();
      if (allowlist === null) {
        return base;
      }
      const allowed = new Set(allowlist);
      const denied = () => Promise.reject(new Error("image_source_excluded"));
      const source = preferences.popupImageSource;
      if (isRecord(source) && source.kind === "dictionary") {
        return (request) =>
          allowed.has(request.dictionary) ? base(request) : denied();
      }
      // Tab-group source: resolve one shared winner in group order.
      let winnerPromise = null;
      const resolveWinner = () => {
        const references = [];
        for (const result of Array.isArray(results) ? results : []) {
          for (const reference of getStructuredMediaReferences(result)) {
            references.push(reference);
          }
        }
        const byDictionary = new Map();
        for (const { dictionary, path } of references) {
          if (!allowed.has(dictionary)) {
            continue;
          }
          if (!byDictionary.has(dictionary)) {
            byDictionary.set(dictionary, []);
          }
          byDictionary.get(dictionary).push(path);
        }
        const probe = async () => {
          for (const dictionary of allowlist) {
            for (const path of byDictionary.get(dictionary) ?? []) {
              try {
                await base({ dictionary, generation, path });
                return dictionary;
              } catch {
                // Missing/stale reference: fall through to the next candidate.
              }
            }
          }
          return null;
      ...
      return (request) => {
        if (!allowed.has(request.dictionary)) {
          return denied();
        }
        if (winnerPromise === null) {
          winnerPromise = resolveWinner();
        }
        return winnerPromise.then((winner) =>
          winner === request.dictionary ? base(request) : denied()
        );
      };
    }
    ...
    function renderTermResults(
      ...
      const rendered = level.view.renderResults(
        results,
        candidate,
        termRenderOptions(targetDepth, dictionaryGeneration, definitionBlurContext, results)
      );
    ...
    /**
     * Renders the projected single-dictionary result for a clicked kanji that
     * was routed to a generic term dictionary. The cached pre-click term view on
     * the level is preserved (not overwritten) so the Back action can restore it,
     * mirroring how the Kanji panel returns to the term results.
     */
    function renderKanjiTermResults(
      results,
      candidate,
      dictionaryGeneration,
      targetDepth,
      ...
      preparePopupContent("kanji_term_results", targetDepth);
      const primaryResult = results[0];
      const level = ensurePopupLevel(targetDepth);
      ...
      const rendered = level.view.renderResults(results, candidate, {
        ...
        onBack: termView ? () => restoreTermView(targetDepth) : null,
        highlightText: termView
      ...
      showPopup(candidate, targetDepth);
      startDefinitionBlurDeadline(definitionBlurContext);
      syncAudioRenderedResults(targetDepth, false);
      void startMiningRefresh(level, rendered.miningItems, rendered.feedback);
    }
    ...
    /**
     * The explicitly selected clicked-kanji dictionary's capability, or null
     * when Automatic (no selection) or the selection is not an installed
     * dictionary. A generic term dictionary (kanjiCount === 0 && termCount > 0)
     * has no kanji bank, so its clicked-kanji lookup must use the term path.
     */
    function selectedKanjiClickDictionaryCapability() {
      const title = preferences.kanjiClickDictionary;
      if (typeof title !== "string" || title.length === 0) {
        return null;
      }
      const presentation = Array.isArray(preferences.dictionaryPresentation)
        ? preferences.dictionaryPresentation
        : [];
      const entry = presentation.find((item) => item && item.title === title);
      if (!entry) {
        return null;
      }
      return {
        title,
        termCount: Number.isFinite(entry.termCount) ? entry.termCount : 0,
        kanjiCount: Number.isFinite(entry.kanjiCount) ? entry.kanjiCount : 0,
      };
    }
    ...
    function requestKanji(character, candidate, targetDepth) {
      ...
      const capability = selectedKanjiClickDictionaryCapability();
      // An explicitly selected generic term dictionary carries no kanji bank, so
      // route the clicked kanji through the term path and render only that
      // dictionary's single-character definition. Automatic clicks and kanji-bank
      // selections keep the existing Kanji request.
      const useTermPath = capability !== null &&
        capability.kanjiCount === 0 &&
        capability.termCount > 0;
      sendLookup(
        candidate,
        latestGeneration,
        targetDepth,
        signature,
        useTermPath ? "term-first" : "kanji",
        kanji,
        useTermPath ? capability.title : null
      );
    }
    ...
    function handleLookupResponse(rawData) {
      ...
      const requestKanjiTermDictionary = latestKanjiTermDictionary;
      ...
      // A clicked kanji routed to an explicitly selected generic term dictionary
      // renders only that dictionary's single-character definition, keeping the
      // cached pre-click term view behind a Back action. Missing matches restore
      // the cached view instead of leaking an unselected dictionary.
      if (typeof requestKanjiTermDictionary === "string") {
        const projected = projectResultsToSelectedDictionary(
          results,
          requestKanjiTermDictionary
        );
        if (projected.length > 0) {
          renderKanjiTermResults(
            projected,
            candidate,
            dictionaryGeneration,
            targetDepth,
          ...
          diagnostic("info", "lookup.kanji-term-rendered", {
            requestId,
            targetDepth,
            dictionary: requestKanjiTermDictionary,
            resultCount: projected.length,
            query: candidate.query,
          });
          return;
        }
        if (popupLevels[targetDepth]?.termView) {
          restoreTermView(targetDepth);
          return;
        }
        latestCandidate = null;
      ...
      if (kanji) {
        ...
        level.view.renderKanji(kanji, candidate, {
          dictionaryPresentation: preferences.dictionaryPresentation,
          kanjiClickDictionary: preferences.kanjiClickDictionary,
    ...
    function sendLookup(
      ...
      latestKanjiTermDictionary = kanjiTermDictionary;
    ...
    function updatePreferences(nextPreferences = {}) {
      ...
      if (
        ...
        !popupImageSourceEqual(
          previous.popupImageSource,
          preferences.popupImageSource
        ) ||
