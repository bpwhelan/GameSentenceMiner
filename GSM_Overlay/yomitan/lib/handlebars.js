...
var require_utils = __commonJS({
  "node_modules/handlebars/dist/cjs/handlebars/utils.js"(exports) {
    ...
    exports.createFrame = createFrame3;
...
var require_helpers = __commonJS({
  "node_modules/handlebars/dist/cjs/handlebars/helpers.js"(exports) {
    ...
    exports.moveHelperToHooks = moveHelperToHooks2;
    function _interopRequireDefault(obj) {
      return obj && obj.__esModule ? obj : { "default": obj };
    }
    ...
    function moveHelperToHooks2(instance, helperName, keepHelper) {
...
var require_proto_access = __commonJS({
  "node_modules/handlebars/dist/cjs/handlebars/internal/proto-access.js"(exports) {
    ...
    exports.resultIsAllowed = resultIsAllowed2;
    ...
    function createProtoAccessControl2(runtimeOptions) {
    ...
    function resultIsAllowed2(result, protoAccessControl, propertyName) {
    ...
    var AST2 = {
      ...
      helpers: {
        ...
        simpleId: function simpleId(path) {
          return path.parts.length === 1 && !AST2.helpers.scopedId(path) && !path.depth;
        }
      }
    };
...
var require_compiler = __commonJS({
  "node_modules/handlebars/dist/cjs/handlebars/compiler/compiler.js"(exports) {
    ...
    function transformLiteralToPath2(sexpr) {
      if (!sexpr.path.parts) {
...
function initData(context, data) {
  if (!data || !("root" in data)) {
    data = data ? (0, import_utils.createFrame)(data) : {};
    data.root = context;
  }
  return data;
}
...
var ElasticHandlebarsVisitor = class extends import_handlebars.default.Visitor {
  ...
  constructor(env, input, options = {}) {
    ...
    __publicField(this, "compileOptions");
    __publicField(this, "runtimeOptions");
    __publicField(this, "blockParamNames", []);
    __publicField(this, "blockParamValues", []);
    __publicField(this, "ast");
    __publicField(this, "container");
    __publicField(this, "defaultHelperOptions");
    __publicField(this, "processedRootDecorators", false);
    // Root decorators should not have access to input arguments. This flag helps us detect them.
    __publicField(this, "processedDecoratorsForProgram", /* @__PURE__ */ new Set());
    this.env = env;
    ...
    const protoAccessControl = (0, import_proto_access.createProtoAccessControl)({});
    const container = this.container = {
      ...
      strict(obj, name, loc) {
        if (!obj || !(name in obj)) {
          throw new import_handlebars.default.Exception('"' + name + '" not defined in ' + obj, {
            loc
          });
        }
        return container.lookupProperty(obj, name);
      ...
      lookupProperty(parent, propertyName) {
        ...
        if ((0, import_proto_access.resultIsAllowed)(result, protoAccessControl, propertyName)) {
          return result;
        }
        return void 0;
      },
  ...
  render(context, options = {}) {
    ...
    (0, import_helpers.moveHelperToHooks)(this.container, "helperMissing", keepHelperInHelpers);
    (0, import_helpers.moveHelperToHooks)(this.container, "blockHelperMissing", keepHelperInHelpers);
    if (!this.ast) {
      this.ast = import_handlebars.default.parse(this.template);
    }
  ...
  PathExpression(path) {
    const blockParamId = !path.depth && !import_ast.default.helpers.scopedId(path) && this.blockParamIndex(path.parts[0]);
    let result;
    if (blockParamId) {
  ...
  classifyNode(node) {
    const isSimple = import_ast.default.helpers.simpleId(node.path);
    const isBlockParam = isSimple && !!this.blockParamIndex(node.path.parts[0]);
    let isHelper = !isBlockParam && import_ast.default.helpers.helperExpression(node);
  ...
  blockParamIndex(name) {
    for (let depth = 0, len = this.blockParamNames.length; depth < len; depth++) {
      ...
      const param = blockParams && (0, import_utils2.indexOf)(blockParams, name);
      if (blockParams && param >= 0) {
        return [depth, param];
      }
    }
  ...
  pushToOutputWithEscapeCheck(result, node) {
    if (!node.escaped || this.compileOptions.noEscape === true || typeof result !== "string") {
      this.output.push(result);
    } else {
      this.output.push(import_handlebars.default.escapeExpression(result));
    }
  }
  ...
  processHelperNode(node) {
    ...
    if (this.compileOptions.knownHelpers && this.compileOptions.knownHelpers[name]) {
      ...
      throw new import_handlebars.default.Exception(
  ...
  invokeHelper(node) {
    ...
    const isSimple = import_ast.default.helpers.simpleId(path);
    const helper = this.setupHelper(node, name);
  ...
  invokePartial(partial) {
    ...
    const name = isDynamic ? this.resolveNodes(partial.name).join("") : partial.name.original;
    ...
    if ("fn" in options && options.fn !== noop) {
      ...
      options.data = (0, import_utils2.createFrame)(options.data);
      partialBlock = options.data["partial-block"] = function partialBlockWrapper(context2, wrapperOptions = {}) {
        wrapperOptions.data = (0, import_utils2.createFrame)(wrapperOptions.data);
        wrapperOptions.data["partial-block"] = currentPartialBlock;
        return fn(context2, wrapperOptions);
      };
      if (fn.partials) {
    ...
    const partialTemplate = this.container.partials[name] ?? partialBlock ?? // TypeScript note: We extend ResolvePartialOptions in our types.ts file
    // to fix an error in the upstream type. When calling back into the
    // upstream code, we just cast back to the non-extended type
    import_handlebars.default.VM.resolvePartial(
      void 0,
      void 0,
      options
    );
    if (partialTemplate === void 0) {
      throw new import_handlebars.default.Exception(`The partial ${name} could not be found`);
    }
    let render;
    ...
    if ("indent" in partial) {
      result = partial.indent + (this.compileOptions.preventIndent ? result : result.replace(/\n(?!$)/g, `
${partial.indent}`));
    }
    this.output.push(result);
  }
  processAmbiguousNode(node) {
    ...
    if (isBlock(node)) {
      const result = helper.fn ? helperResult : this.container.hooks.blockHelperMissing.call(this.context, helperResult, helper.options);
      if (result != null) {
        this.output.push(result);
      }
    } else {
      if (node.escaped === false || this.compileOptions.noEscape === true || typeof helperResult !== "string") {
        this.output.push(helperResult);
      } else {
        this.output.push(import_handlebars.default.escapeExpression(helperResult));
      }
    }
  }
  ...
  setupParams(node, name) {
    ...
    if (isBlock(node)) {
      options.fn = node.program ? this.processDecorators(node.program, this.generateProgramFunction(node.program)) : noop;
      options.inverse = node.inverse ? this.processDecorators(node.inverse, this.generateProgramFunction(node.inverse)) : noop;
    }
    return options;
  }
...
import_handlebars2.default.create = function() {
  ...
  SandboxedHandlebars.compileAST = import_handlebars2.default.compileAST;
...
var export_Handlebars = import_handlebars2.default;
export {
  export_Handlebars as Handlebars
};
