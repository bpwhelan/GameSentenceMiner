...
export interface HoshidictsReaderPreferencesRequest {
    ...
    kanjiClickDictionary: string | null;
    popupImageSource: HoshidictsPopupImageSource | null;
...
/**
 * Selects which dictionary (or ordered tab group) is permitted to contribute
 * images to the popup. `null` keeps the legacy Automatic behaviour where every
 * dictionary in a result may show its own images.
 */
export type HoshidictsPopupImageSource =
    | { kind: 'dictionary'; title: string }
    | { kind: 'tabGroup'; id: string };

/**
 * The subset of a normalized overlay lookup result the selected-dictionary
 * projection needs. Kept structural so both the shared unit tests and the
 * overlay reader can describe their results without importing each other.
 */
export interface HoshidictsProjectableGlossary {
    dictionary: string;
    glossary: string;
...
export interface HoshidictsProjectableResult {
    term: {
        glossaries: HoshidictsProjectableGlossary[];
        [key: string]: unknown;
    };
    [key: string]: unknown;
}
...
/**
 * Projects a term lookup to a single explicitly selected dictionary: each
 * result keeps only the glossaries whose `dictionary` matches `title`, and
 * results left with no matching glossary are dropped. The source results and
 * their glossary arrays are never mutated, so the cached term view they came
 * from stays intact for a Back action.
 */
export function projectHoshidictsResultsToSelectedDictionary<
    T extends HoshidictsProjectableResult,
>(results: readonly T[], title: string): T[] {
    const projected: T[] = [];
...
const HOSHIDICTS_READER_PREFERENCE_SPECS: readonly HoshidictsReaderPreferenceSpec[] =
    [
        ...
        dictionaryTitlePreference(
            ...
            'Hoshidicts kanji click dictionary is invalid.',
            { trim: true }
        ),
        {
            key: 'popupImageSource',
            createDefault: () => null,
            validate: validatePopupImageSource,
            canonicalize: (value) =>
                canonicalizePopupImageSource(
                    value as HoshidictsPopupImageSource | null
                ),
            equals: (left, right) =>
                popupImageSourceEqual(
                    left as HoshidictsPopupImageSource | null,
                    right as HoshidictsPopupImageSource | null
                ),
        },
        booleanPreference(
...
const MAX_HOSHIDICTS_POPUP_IMAGE_SOURCE_LENGTH = 4096;

/**
 * Validates the discriminated popup image source. `null` (Automatic) is always
 * acceptable; a dictionary source needs a non-blank bounded title; a tab-group
 * source needs a non-blank bounded id.
 */
function validatePopupImageSource(value: unknown): string | null {
    const message = 'Hoshidicts popup image source is invalid.';
    if (value === null) {
        return null;
    }
    if (!isHoshidictsRecord(value)) {
    ...
    if (value.kind === 'dictionary') {
        return typeof value.title === 'string' &&
            ...
            value.title.length <= MAX_HOSHIDICTS_POPUP_IMAGE_SOURCE_LENGTH
            ? null
            : message;
    }
    if (value.kind === 'tabGroup') {
        return typeof value.id === 'string' &&
            ...
            value.id.length <= MAX_HOSHIDICTS_POPUP_IMAGE_SOURCE_LENGTH
            ? null
            : message;
    }
    return message;
...
/** Copies an already-valid source into its canonical two-key shape. */
function canonicalizePopupImageSource(
    value: HoshidictsPopupImageSource | null
): HoshidictsPopupImageSource | null {
    if (value === null) {
        return null;
    }
    if (value.kind === 'dictionary') {
        return { kind: 'dictionary', title: value.title.trim() };
    }
    return { kind: 'tabGroup', id: value.id };
}

function popupImageSourceEqual(
    left: HoshidictsPopupImageSource | null,
    right: HoshidictsPopupImageSource | null
): boolean {
    ...
    return left.kind === 'dictionary'
        ? left.title === (right as { title: string }).title
        : left.id === (right as { id: string }).id;
}

/**
 * Maps a popup image source to the ordered list of dictionary titles that are
 * allowed to contribute images. `null` (Automatic) returns `null`, meaning
 * every dictionary is permitted. An individual source yields its single title.
 * A tab-group source yields the group's members in their established order; a
 * missing or deleted group yields an empty allowlist so no images are shown.
 */
export function resolveHoshidictsPopupImageSourceDictionaries(
    source: HoshidictsPopupImageSource | null,
    tabGroups: readonly HoshidictsReaderTabGroup[]
): string[] | null {
    if (source === null) {
        return null;
    }
    ...
    const group = tabGroups.find((candidate) => candidate.id === source.id);
    return group ? [...group.dictionaries] : [];
}
...
export function hoshidictsReaderPreferencesFromSnapshot(
    ...
    return {
        ...
        dictionaryPresentation: (snapshot.dictionaries ?? []).map(
            ({ title, displayName, favorite, frequencyMode, termCount, kanjiCount, mediaCount }) => {
