...
describe('Hoshidicts reader preferences', () => {
    ...
    // The full range table lives in shared/features/hoshidicts.test.ts, beside
    // the validator that owns these bounds. These two pin that the manager
    // surfaces its errors rather than swallowing them.
    it.each([
...
describe('Hoshidicts updates and schedule', () => {
    ...
    it('continues updating dictionaries after one update fails', async () => {
        const baseDir = makeTempDir();
        const archivesDir = makeTempDir();
        const alpha = writeArchive(archivesDir, 'alpha.zip', {
            title: 'Alpha',
        ...
        const fetchRemoteIndex = vi.fn(async (url: string) => {
            if (url.includes('alpha')) {
                throw new Error('Alpha update index unavailable');
            }
            return {
                revision: 'beta-two',
                downloadUrl: 'https://cdn.example/beta.zip',
            };
        });
        const downloadArchive = vi.fn(async (_url: string, outputPath: string) => {
            fs.mkdirSync(path.dirname(outputPath), { recursive: true });
            fs.writeFileSync(
                outputPath,
                JSON.stringify({ ...readArchive(beta), revision: 'beta-two' }),
                'utf8'
            );
        });
        const { manager } = createHarness(baseDir, {
        ...
        await manager.importDictionaries([alpha, beta]);
        ...
        expect(snapshot.dictionaries.map(({ revision }) => revision)).toEqual([
            'alpha-one',
            'beta-two',
        ]);
        expect(snapshot.lastError).toContain(
            'Alpha: Alpha update index unavailable'
        );
    });
