"""
Property-based test for the /api/tokenisation/words/not-in-anki endpoint.
...
class _FreqTestContext:
    ...
    def setup(self):
        ...
        # Ensure the tokenised column exists (set_db creates the table but
        # tokenised may not be in _fields)
        try:
            self.db.execute(
    ...
    **Validates: Requirements 2.3**
    ...
    try:
        ...
        with patch(
            ...
            resp = ctx.client.get(
                f"/api/tokenisation/words/not-in-anki?limit=1000&offset=0"
            )
