...
def _create_line(game_name="Test Game", text="テスト文", timestamp=None, game_id="",
                 screenshot_in_anki="", audio_in_anki=""):
    line = GameLinesTable(
        id=str(uuid.uuid4()),
        game_name=game_name,
        game_id=game_id,
...
class TestDeduplication:
    def test_preview_finds_duplicates_in_time_window(self, client):
        ...
        resp = client.post("/api/preview-deduplication", json={
            "games": ["Game"],
...
class TestSearchDuplicates:
    def test_finds_duplicates(self, client):
        ...
        resp = client.post("/api/search-duplicates", json={
            "game": "G",
            "time_window_minutes": 5,
        })
        data = resp.get_json()
...
class TestMergeGames:
    def test_merge_success(self, client):
        ...
        resp = client.post("/api/merge_games", json={
            "target_game": "Target",
...
class TestMigrateLines:
    def test_migrate_lines_success(self, client):
        ...
        resp = client.post("/api/migrate-lines", json={
